...
def main():
    parser = argparse.ArgumentParser(description='Create onnx file from yolov3 config and weight file.')
    parser.add_argument('cfg_file_path', help='yolov3 cfg file path', type=str)
    parser.add_argument('weights_file_path', help='yolov3 weights file path', type=str)
    parser.add_argument('label_file_path', help='yolov3 label file path', type=str)
    parser.add_argument('--input_size', help='input size for yolov3', default=256, type=int)
    parser.add_argument('--output_file_path', '-o', help='output file path', default='yolov3.onnx', type=str)
    opt = parser.parse_args()

    cfg_file_path = opt.cfg_file_path
    ...
    del parser
    # count class num
    categories = [line.rstrip('\n') for line in open(opt.label_file_path)]
    class_num = len(categories)
    # In above layer_config, there are three outputs that we need to know the output
    # shape of (in CHW format):
    output_tensor_dims = OrderedDict()
    output_tensor_dims['082_convolutional'] = [3 * (class_num + 5), int(opt.input_size / 32), int(opt.input_size / 32)]
    output_tensor_dims['094_convolutional'] = [3 * (class_num + 5), int(opt.input_size / 16), int(opt.input_size / 16)]
    output_tensor_dims['106_convolutional'] = [3 * (class_num + 5), int(opt.input_size / 8), int(opt.input_size / 8)]

    # Create a GraphBuilderONNX object with the known output tensor dimensions:
    builder = GraphBuilderONNX(output_tensor_dims)

    weights_file_path = opt.weights_file_path
