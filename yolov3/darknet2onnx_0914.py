...
def main():
    parser = argparse.ArgumentParser(
        description='Create onnx file from yolov3 config and weight file.')
    parser.add_argument('cfg_file_path', help='yolov3 cfg file path', type=str)
    parser.add_argument('weights_file_path',
                        help='yolov3 weights file path', type=str)
    parser.add_argument('label_file_path',
                        help='yolov3 label file path', type=str)
    parser.add_argument(
        '--input_size', help='input size for yolov3', default=256, type=int)
    parser.add_argument('--output_file_path', '-o',
                        help='output file path', default='yolov3.onnx', type=str)
    ...
    #     3 * (class_num + 5), int(opt.input_size / 32), int(opt.input_size / 32)]
    # output_tensor_dims['094_convolutional'] = [
    #     3 * (class_num + 5), int(opt.input_size / 16), int(opt.input_size / 16)]
    #注意output node name是098_convolutional而不是98_convolutional. name错了将无法生成正确的拓扑图!
    output_tensor_dims['098_convolutional'] = [
        3 * (class_num + 5), int(opt.input_size / 4), int(opt.input_size / 4)]
